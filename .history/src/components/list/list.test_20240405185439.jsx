...
describe('List component', () => {
  ...
  test('marks a task as completed', () => {
    const { getByText, getByTestId, getByPlaceholderText } = render(<List />);
    const addButton = getByText('Add Task')
    fireEvent.click(addButton)
    ...
    const checkButton = getByTestId(`check-0`)

    const [completedTasks] = React.useState([]);
    expect(completedTasks.includes(0)).toBe(true);
  });
